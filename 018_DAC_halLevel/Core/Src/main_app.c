...
int main(void){
	...
	uint32_t value = 0;
	uint8_t direction = 1; // 1: Ramp Up, 0: Ramp Down

	while(TRUE){

		// Write the digital value to the DAC output register (12-bit Right Aligned)
		// This converts the number (0-4095) to Voltage (0V - 3.3V)
		...
		if(direction){
			...
			if(value >= 4095){ // Check upper limit (12-bit max)
				value = 4095;
				direction = 0; // Switch to Ramp Down
			}
		}
		else{
			// Ramp Down
...
// Configure DAC1 Instance and Channel
void DAC1_Config(void){
	...
	sConfig.DAC_Trigger = DAC_TRIGGER_NONE; // No hardware trigger, software control
	sConfig.DAC_OutputBuffer = DAC_OUTPUTBUFFER_ENABLE; // Enable buffer to drive loads
...
// Low-Level Hardware Initialization (GPIO & Clock)
void DAC_LowLevel_Config(void){
	...
	// Configure PA4 as Analog (DAC1_OUT1)
